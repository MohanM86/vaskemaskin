...
export function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return createMeta({
    title: 'Hvitevarebutikker i ' + kommune.name + ' | ' + stores.length + ' butikker',
    description: 'Finn hvitevarebutikker som selger vaskemaskiner i ' + kommune.name + '. ' + stores.length + ' butikker med adresser og kontaktinfo.',
    path: 'kommune/' + kommune.slug,
  })
}
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLdBreadcrumb([
        ...
        { name: fylke?.name || '', url: 'https://vaskemaskin.no/fylke/' + kommune.fylkeSlug + '/' },
        { name: kommune.name, url: 'https://vaskemaskin.no/kommune/' + kommune.slug + '/' },
      ])) }} />
      <div className="container-site py-10">
        ...
        <p className="text-lg text-slate-600 mb-8">{stores.length} butikker som selger vaskemaskiner i {kommune.name} kommune, {fylke?.name}.</p>
        ...
        <section className="mb-12 p-6 rounded-xl bg-aqua-50 border border-aqua-200">
          <h3 className="font-serif text-xl font-bold text-slate-900 mb-2">Trenger du ny vaskemaskin i {kommune.name}?</h3>
          <p className="text-sm text-slate-600 mb-4">Les vår komplette kjøpsguide for å finne den modellen som passer best for dine behov.</p>
          <Link href="/artikkel/komplett-kjopsguide-2026/" className="btn-primary inline-flex items-center gap-2">Les kjøpsguiden</Link>
        </section>
        ...
        {naboKommuner.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {naboKommuner.slice(0, 12).map(k => (
                <Link key={k.slug} href={'/kommune/' + k.slug + '/'} className="p-3 rounded-lg border border-slate-200 hover:border-aqua-300 text-sm text-slate-700 transition-colors">{k.name}</Link>
              ))}
            </div>
          </section>
        )}
