...
export function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  if (!store) return {}
  return createMeta({
    title: store.name + ' | Hvitevarebutikk i ' + store.kommune,
    description: store.name + ' i ' + store.kommune + '. Hvitevarebutikk med adresse ' + store.address + ', ' + store.postnummer + ' ' + store.poststed + '.',
...
export default function ButikkPage({ params }: { params: { slug: string } }) {
  const store = getStoreBySlug(params.slug)
  if (!store) notFound()
  const kommune = getKommuneBySlug(store.kommuneSlug)
  ...
  const localBizSchema = {
    ...
    address: {
      ...
      addressRegion: store.fylke,
      addressCountry: 'NO',
    },
    ...(store.ansatte > 0 ? { numberOfEmployees: { '@type': 'QuantitativeValue', value: store.ansatte } } : {}),
  }
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLdBreadcrumb([
        ...
        { name: store.fylke, url: 'https://vaskemaskin.no/fylke/' + store.fylkeSlug + '/' },
        { name: store.kommune, url: 'https://vaskemaskin.no/kommune/' + store.kommuneSlug + '/' },
        { name: store.name, url: 'https://vaskemaskin.no/butikk/' + store.slug + '/' },
      ])) }} />
      <div className="container-site py-10">
        <Breadcrumbs items={[
          { label: 'Hjem', href: '/' },
          { label: store.fylke, href: '/fylke/' + store.fylkeSlug + '/' },
          { label: store.kommune, href: '/kommune/' + store.kommuneSlug + '/' },
          { label: store.name },
        ]} />
        ...
        <div className="max-w-3xl">
          ...
          <div className="p-6 rounded-xl bg-aqua-50 border border-aqua-200 mb-8">
