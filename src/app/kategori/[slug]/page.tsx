  ...
  return (
    <>
      ...
      <div className="container-site py-10">
        ...
        <div className="prose-article max-w-3xl mb-12"><p className="text-base text-slate-700 leading-relaxed">{cat.intro}</p></div>
        {catArticles.length > 0 && (<section className="mb-14"><h2 className="font-serif text-2xl font-bold text-slate-900 mb-6">Artikler om {cat.shortName.toLowerCase()}</h2><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">{catArticles.map(article => (<ArticleCard key={article.slug} article={article} />))}</div></section>)}
        <section className="mb-10"><h2 className="font-serif text-2xl font-bold text-slate-900 mb-6">Andre kategorier</h2><div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{otherCategories.map(c => (<a key={c.slug} href={'/kategori/' + c.slug + '/'} className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 hover:border-aqua-300 transition-colors"><div className="w-8 h-8 rounded-lg bg-aqua-50 flex items-center justify-center text-aqua-600"><CategoryIcon slug={c.slug} size={16} /></div><span className="text-sm font-medium text-slate-700">{c.shortName}</span></a>))}</div></section>
      </div>
    </>
  )
}
