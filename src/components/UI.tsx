...
export function Breadcrumbs({ items }: { items: { label: string; href?: string }[] }) {
  return (
    <nav aria-label="Brødsmuler" className="mb-6 text-sm text-slate-500">
      <ol className="flex flex-wrap items-center gap-1.5">
        {items.map((item, i) => (
          <li key={i} className="flex items-center gap-1.5">
            {i > 0 && <span className="text-slate-300">/</span>}
            {item.href ? (
              <Link href={item.href} className="hover:text-aqua-700 transition-colors">{item.label}</Link>
            ) : (
...
export function FAQSection({ items, heading }: { items: { question: string; answer: string }[]; heading?: string }) {
  return (
    <section className="mt-12">
      <h2 className="section-heading text-2xl mb-6">{heading || 'Ofte stilte spørsmål'}</h2>
...
function FAQItem({ question, answer }: { question: string; answer: string }) {
  return (
    <details className="group border border-slate-200 rounded-lg overflow-hidden bg-white">
      ...
      <div className="px-4 pb-4 text-sm text-slate-600 leading-relaxed">{answer}</div>
    </details>
  )
}
...
export function ArticleCard({ article }: { article: Article }) {
  return (
    <Link href={'/artikkel/' + article.slug + '/'} className="card group block">
      <div className="p-5">
        ...
        <h3 className="font-serif text-lg font-bold text-slate-900 group-hover:text-aqua-700 transition-colors line-clamp-2 mb-2">{article.title}</h3>
