      ...
      <div className="container-site">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            <div className="relative" onMouseEnter={() => handleMouseEnter('kategorier')} onMouseLeave={handleMouseLeave}>
            ...
            <div className="relative" onMouseEnter={() => handleMouseEnter('merker')} onMouseLeave={handleMouseLeave}>
              ...
              {activeMenu === 'merker' && (
                <div className="absolute top-[calc(100%+12px)] left-1/2 -translate-x-1/2 w-[400px] bg-white rounded-xl shadow-xl border border-slate-200 p-5">
                  <div className="grid grid-cols-2 gap-2">
                    {brands.map(brand => (
                      <Link key={brand.slug} href={'/merke/' + brand.slug + '/'} className="flex items-center gap-2 p-2.5 rounded-lg hover:bg-aqua-50 transition-colors">
                        <div className="w-7 h-7 rounded-md bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600">{brand.name.charAt(0)}</div>
                        <div>
            ...
            <div className="relative" onMouseEnter={() => handleMouseEnter('artikler')} onMouseLeave={handleMouseLeave}>
                ...
                <div className="absolute top-[calc(100%+12px)] right-0 w-[360px] bg-white rounded-xl shadow-xl border border-slate-200 p-5">
                  <div className="space-y-1">
                    {latestArticles.map(article => (
                      <Link key={article.slug} href={'/artikkel/' + article.slug + '/'} className="block p-3 rounded-lg hover:bg-aqua-50 transition-colors">
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-100">
                    <Link href="/artikkel/komplett-kjopsguide-2026/" className="text-sm font-semibold text-aqua-600 hover:text-aqua-700">Se alle artikler</Link>
                  </div>
                </div>
              )}
            </div>

            <Link href="/om-oss/" className="px-3 py-2 text-sm font-medium text-slate-700 hover:text-aqua-700 hover:bg-slate-50 rounded-lg transition-colors">Om oss</Link>
          </nav>
        ...
        <div className="lg:hidden border-t border-slate-100 bg-white">
          <div className="container-site py-4 space-y-4">
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {categories.map(cat => (
                  <Link key={cat.slug} href={'/kategori/' + cat.slug + '/'} className="flex items-center gap-2 p-2 rounded-lg hover:bg-aqua-50 text-sm text-slate-700" onClick={() => setMobileOpen(false)}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {brands.slice(0, 8).map(brand => (
                  <Link key={brand.slug} href={'/merke/' + brand.slug + '/'} className="p-2 rounded-lg hover:bg-aqua-50 text-sm text-slate-700" onClick={() => setMobileOpen(false)}>{brand.name}</Link>
                ))}
              </div>
            </div>
            <div className="pt-2 border-t border-slate-100">
              <Link href="/om-oss/" className="block p-2 text-sm text-slate-700" onClick={() => setMobileOpen(false)}>Om oss</Link>
            </div>
          </div>
        </div>
      )}
